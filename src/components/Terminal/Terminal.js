...
export default class Terminal extends Component {
    ...
    renderHistoryItem() {
        return (item, key) => {
            const prefix = item.hasOwnProperty('cwd') ? (
                <span className="prefix">{`${this.props.prefix} ~${item.cwd} $`}</span>
            ) : undefined;
    ...
    render() {
        ...
        return (
            <div id="ReactBash">
                <div className="TerminalBody" onClick={() => this.refs.input.focus()}>
                    {history.map(this.renderHistoryItem())}
                    <form className="TerminalForm" onSubmit={evt => this.handleSubmit(evt)}>
                        <span className="prefix">{`${prefix} ~${cwd} $`}</span>
                        <input
                          autoComplete="off"
